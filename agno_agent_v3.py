...
from agent import PolicyAgentV2, PolicyAgent
...
lg_agent = LangGraphAgent(
    ...
    input_key="messages",
    output_key="messages",
    config= {"configurable": {"vectorstore": _init_vectorstore(postgres_uri, embeddings)}}
)

# ── AgentOS lifespan (builds graph + wires everything) ────────────────
@asynccontextmanager
async def agno_lifespan(app, agent_os):
    load_dotenv()
...
_base_app.state.limiter = limiter
_base_app.add_middleware(SlowAPIMiddleware)


@_base_app.exception_handler(RateLimitExceeded)
...
@_base_app.get("/health")
async def health():
    return {
        "status": "healthy",
        "components": {
            "graph": _base_app.state.graph is not None,
            "vectorstore": _base_app.state.vectorstore is not None,
        },
    }
...
# # ── Chat endpoints ─────────────────────────────────────────────────────
# @_base_app.post("/chat", response_model=ChatResponse)
# @limiter.limit("30/minute")
# async def chat(request: Request, req: ChatRequest):
#     thread_id = _resolve_thread_id(req.thread_id)
#     config = _get_config(thread_id, _base_app.state.vectorstore)
#
#     try:
#         t_invoke = time.perf_counter()
#         result = await asyncio.wait_for(
#             _base_app.state.graph.ainvoke({"messages": [HumanMessage(content=req.message)]}, config),
#             timeout=60.0,
#         )
...
#             async for msg_chunk, metadata in _base_app.state.graph.astream(
